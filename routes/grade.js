...
function getLateTakers(cid) {
  if (cid == 3)
    return [
      1001731, 1001623, 1001620, 1001475, 1002158, 1001944, 1001092, 1002595,
      1001904, 1001642,
    ]; // anush devkar, armaan ahmed, anusha agarwal, kanishk sivanadam, max zhao, rishikesh narayana, samarth bhargav, nathan liang, esha m, navya arora
...
router.get("/status/:id", checkLoggedIn, async (req, res) => {
  ...
  if (vals.user == req.session.userid || req.session.admin) {
    if (
      ...
      vals.insight.startsWith("Viewing as admin")
    ) {
      ...
      for (let useri = 0; useri < rankings.length; useri++) {
        ...
        for (let j = 0; j < standings.load.length; j++) {
          if (rankings[useri].id == standings.load[j].id) {
            rankings[useri].inhouses.push(
              (2000 * (standings.load.length - standings.load[j].rank + 1)) /
                standings.load.length
            );
            took = true;
            break;
          }
