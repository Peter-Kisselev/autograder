...
async function grabAllProblems(isAdmin) {
  return new Promise((resolve, reject) => {
    pl.connect((err, client, release) => {
      ...
      let qry =
        "SELECT * FROM problems WHERE (problems.secret IS NULL OR problems.secret = false OR problems.secret = $1)";
      let params = [];
      if (isAdmin) {
        params.push(true);
      } else {
  ...
  return new Promise((resolve, reject) => {
    pl.connect((err, client, release) => {
      ...
      if (contest == undefined && user == undefined) {
        ...
        qry =
          "SELECT * FROM submissions WHERE submissions.usr = $1 ORDER BY id ASC;";
        params.push(user);
      } else if (user == undefined) {
        qry =
          "SELECT * FROM submissions WHERE submissions.contest = $1 ORDER BY id ASC;";
        params.push(contest);
      } else {
        qry =
          "SELECT * FROM submissions WHERE submissions.usr = $1 AND submissions.contest = $2 ORDER BY id ASC;";
        params.push(user);
        params.push(contest);
      }
      client.query(qry, params, (err, results) => {
...
async function insertSubmission(id, verdict, runtime, memory, insight) {
  return new Promise((resolve, reject) => {
    pl.connect((err, client, release) => {
      ...
      client.query(
        qry,
        [verdict, runtime, memory, insight, id],
        (err, results) => {
          release();
          if (err) {
            console.log(
              "An error occured while querying to insert submission",
              err
            );
            resolve(false);
          }
  ...
  return new Promise((resolve, reject) => {
    pl.connect((err, client, release) => {
      ...
      client.query(qry, vals, (err, results) => {
        ...
        if (err) {
          console.log(
            "An error occured while querying to create submission",
            err
          );
          resolve(false);
        } else if (results.rows.length == 0) {
  ...
  return new Promise((resolve, reject) => {
    pl.connect((err, client, release) => {
      ...
      client.query(qry, [cid], (err, results) => {
        ...
        if (err) {
          console.log(
            "An error occured while querying for contest problems",
            err
          );
          resolve(false);
        }
  ...
  return new Promise((resolve, reject) => {
    pl.connect((err, client, release) => {
      ...
      client.query(qry, [usaco, id], (err, results) => {
        ...
        if (err) {
          console.log(
            "An error occured while querying to update usaco division",
            err
          );
        }
        resolve(true);
  ...
  return new Promise((resolve, reject) => {
    pl.connect((err, client, release) => {
      ...
      client.query(qry, [cf, id], (err, results) => {
        ...
        if (err) {
          console.log(
            "An error occured while querying to update cf handle",
            err
          );
          resolve(false);
        }
  ...
  return new Promise((resolve, reject) => {
    pl.connect((err, client, release) => {
      ...
      client.query(qry, [], (err, results) => {
        if (err) {
          console.log(
            "An error occured while querying to update cf rating",
            err
          );
          resolve(false);
        }
        ...
        fetch(url)
          ...
          .then((data) => {
            ...
            for (let i = 0; i < results.rows.length; i++) {
              ...
              client.query(
                qry2,
                [data[i].maxRating, results.rows[i].id],
                (err, res) => {
                  if (err) {
                    console.log(
                      "An error occured while querying to update cf rating",
...
module.exports = {
  grab: (id) => {
    if (Number(id)) return grab(id);
    return;
  },
  grabChecker: (id) => {
    if (Number(id)) return grabChecker(id);
  },
  ...
  grabStatus: (id) => {
    if (Number(id)) return grabStatus(id);
    return;
  },
  grabProblem: (id) => {
    if (Number(id)) return grabProblem(id);
    return;
  },
  grabTests: (id) => {
    if (Number(id)) return grabTests(id);
    return;
  },
  grabProfile: (id) => {
    if (Number(id)) return grabProfile(id);
    return false;
  },
  ...
  grabContestProblems: (cid) => {
    if (Number(cid)) return grabContestProblems(cid);
    return;
  },
